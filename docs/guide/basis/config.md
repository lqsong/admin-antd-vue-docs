...
`admin-antd-vue` 内置了一个站点配置文件 `@/config/settings.ts`。
...
`admin-antd-vue` 独立出了一个路由入口配置文件 `@/config/routes.ts`，其目的主要是：统一引入`@/layouts`下不同`layout`的路由，集中处理重新格式化路由。
...
`admin-antd-vue` 集成了 `Vuex`，配置文件 `@/config/store.ts` 实现了自动导入功能，并制定了 `StoreModule规则`。
...
`admin-antd-vue` 包含了国际化功能，并实现了自动导入功能，主要归功于国际化入口配置文件 `@/config/i18n.ts`。
...
`admin-antd-vue` 基于 `vue-cli`来进行构建，所以有个 vue-cli 配置文件 `/vue.config.js`。

[官方文档](https://cli.vuejs.org/zh/config/)


## 环境变量
`admin-antd-vue` 基于 `vue-cli`来进行构建，所以有环境变量配置文件 `/.env.development`、`/.env.production`。
